	...
	bool debug)
{
	...
	CHECK_CUDA(FORWARD::render(
		...
		background,
		out_color, out_normal, out_depth, out_depth_buffer, out_opac, out_opac_buffer,
		blend_counts,
		config), debug)
	return num_rendered;
}
...
void CudaRasterizer::Rasterizer::backward(
    ...
    const float* dL_dpixdepth_buffer,   // Add input depth buffer gradient
	const float* dL_dpixopac,
    const float* dL_dpixopac_buffer,    // Add input opacity buffer gradient
	const int* blend_counts,
	float* dL_dmean2D,
	float* dL_dconic,
	float* dL_dopacity,
	float* dL_dcolor,
