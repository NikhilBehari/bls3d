...
renderCUDA(
	...
	float* __restrict__ out_depth_buffer,
	float* __restrict__ out_opac,
	float* __restrict__ out_opac_buffer,
	int* __restrict__ blend_counts,
	...
	float* config)
{
	...
	if (inside)
	{
		...
		for(int bui = 0; bui < 128; bui++){
			out_depth_buffer[bui * H * W + pix_id] = D_buffer[bui];
		}
		out_opac[pix_id] = 1 - T;
		for(int bui = 0; bui < 128; bui++){
			out_opac_buffer[bui * H * W + pix_id] = W_buffer[bui];
		}
		if (normalize_depth) final_D[pix_id] = D;

		blend_counts[pix_id] = min(blend_count, D_buffer_size);
	...
	float* config)
{
	renderCUDA<NUM_CHANNELS> << <grid, block >> > (
		...
		n_contrib_cut,
		bg_color,
		out_color, out_normal, out_depth, out_depth_buffer, out_opac, out_opac_buffer, blend_counts, //out_rayVar, 
		config);
}
