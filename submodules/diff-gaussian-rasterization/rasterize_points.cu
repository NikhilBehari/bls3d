...
RasterizeGaussiansCUDA(
	...
	const torch::Tensor& prcppoint,
	const torch::Tensor& patchbbox,
	...
	const torch::Tensor& config)
{
  ...
  torch::Tensor out_depth_buffer = torch::full({128, H, W}, 0.0, float_opts);
  torch::Tensor out_opac = torch::full({1, H, W}, 0.0, float_opts);
  torch::Tensor out_opac_buffer = torch::full({128, H, W}, 0.0, float_opts);
  torch::Tensor radii = torch::full({P}, 0, means3D.options().dtype(torch::kInt32));
  torch::Tensor blend_counts = torch::full({1, H, W}, 0, means3D.options().dtype(torch::kInt32));
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		prcppoint.contiguous().data<float>(),
		patchbbox.contiguous().data<float>(),
		...
		out_depth_buffer.contiguous().data<float>(),
		out_opac.contiguous().data<float>(),
		out_opac_buffer.contiguous().data<float>(),
		blend_counts.contiguous().data<int>(),
		radii.contiguous().data<int>(),
		debug);
  }
  return std::make_tuple(rendered, out_color, out_normal, out_depth, out_depth_buffer, out_opac, out_opac_buffer,  blend_counts, radii, geomBuffer, binningBuffer, imgBuffer);
}
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
 RasterizeGaussiansBackwardCUDA(
	...
	const torch::Tensor& dL_dout_depth_buffer,  // Add depth buffer gradient
    const torch::Tensor& dL_dout_opac,
	const torch::Tensor& dL_dout_opac_buffer,   // Add opacity buffer gradient
	const torch::Tensor& blend_counts,  
	const torch::Tensor& sh,
	const int degree,
	const torch::Tensor& campos,
	...
	const torch::Tensor& config) 
{
  ...
  torch::Tensor dL_ddepth = torch::zeros({P, 1}, means3D.options());
  torch::Tensor dL_dconic = torch::zeros({P, 2, 2}, means3D.options());
  torch::Tensor dL_dopacity = torch::zeros({P, 1}, means3D.options());
  torch::Tensor dL_dcutoff = torch::zeros({P, 4}, means3D.options());
  ...
  {  
	  CudaRasterizer::Rasterizer::backward(P, degree, M, R,
		...
		dL_dout_depth_buffer.contiguous().data<float>(),  // Pass depth buffer gradient
		dL_dout_opac.contiguous().data<float>(),
		dL_dout_opac_buffer.contiguous().data<float>(),   // Pass opacity buffer gradient
		blend_counts.contiguous().data<int>(),
