...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, patch_size: list, scaling_modifier = 1.0, override_color = None):
    ...
    if len(outputs) == 7:
        rendered_image, rendered_normal, rendered_depth, rendered_depth_buffer, rendered_opac, rendered_opac_buffer, radii = outputs
    elif len(outputs) == 5:
        rendered_image, rendered_normal, rendered_depth, rendered_opac, radii = outputs
        rendered_depth_buffer, rendered_opac_buffer = None, None
    else:
