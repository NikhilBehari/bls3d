...
def loadCam(args, id, cam_info, resolution_scale, scene_scale, camera_lr):
    ...
    if cam_info.transi is not None:
        if len(cam_info.transi.shape) == 2:
            resized_transi = cam_info.transi.transpose(1, 0)
        else:
            resized_transi = cam_info.transi.transpose(2, 0, 1)
    else:
        resized_transi = None
    ...
    return Camera(colmap_id=cam_info.uid, R=cam_info.R, T=cam_info.T, 
                  ...
                  mask=resized_mask, transi=resized_transi, mono=resized_mono, scene_scale=scene_scale, camera_lr=camera_lr)
