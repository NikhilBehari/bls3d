...
def batch_histogram(timing_data, weights, t_min, t_max, num_bins):
        ...
        timing_quant = torch.clip(torch.round((timing_data-t_min)/bin_width).long(),max=num_bins-1)
...
def render_set(model_path, use_mask, name, iteration, views, gaussians, pipeline, background, write_image, poisson_depth):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        image, normal, depth, depth_buffer, opac, opac_buffer, viewspace_point_tensor, visibility_filter, radii = \
        ...
        if write_image:
            ...
            # save depth and normal 
            np.save(os.path.join(render_path, '{}'.format(view.image_name) + f"_depth.npy"), depth[0].cpu().numpy())
            save_image(normal_wrt.cpu(), os.path.join(render_path, '{}'.format(view.image_name) + "_normal.png"))
