...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, prcppoint=prcppoint, image=image,
                              image_path=image_path, image_name=image_name, width=width, height=height, mask=mask, mono=mono, transi=transi)
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            cam_infos.append(CameraInfo(uid=idx, R=R, T=T, FovY=FovY, FovX=FovX, prcppoint=prcppoint, image=image,
                            ...
                            mask=mask, transi=transi, mono=mono))
    # except FileNotFoundError:
    #     print(os.path.join(path, transformsfile))
    #     print(f"{transformsfile} not found!")
    return cam_infos

def readNerfSyntheticInfo(path, white_background, eval, images_used, extension=".png"):
    print("Reading Training Transforms")
    train_cam_infos = readCamerasFromTransforms(path, "transforms_train.json", white_background, extension)[:images_used]
    ...
    if os.path.exists(ply_path):
        ...
        # we generate random points inside the bounds of the synthetic Blender scenes
        rand_scale = 2
    ...
    # original code loading ply below
    # try:
    #     pcd = fetchPly(ply_path)
    # except:
    #     pcd = None
...
def readIDRCameras(path):
    ...
    transi_dir = '{0}/transient'.format(path)
    transi_paths = sorted(glob_npys(transi_dir))
    ...
    object_transis = []
    for i in transi_paths: 
        object_transi = load_transi(i)
        object_transis.append(object_transi[None])
    ...
    for i in range(n_images):
        ...
        cam_infos.append(CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, prcppoint=prcppoint, image=image,
                        image_path=image_path, image_name=image_name, width=image.size[0], height=image.size[1],
                        mask = object_masks[i], transi = object_transis[i], mono=mono))
    return cam_infos
