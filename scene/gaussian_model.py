...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        # scales = torch.log(torch.sqrt(dist2 / 4))[...,None].repeat(1, 3) * 3
    ...
    def cat_tensors_to_optimizer(self, tensors_dict):
        optimizable_tensors = {}
        for group in self.optimizer.param_groups:
            # Skip if the parameter is a scalar
            if group["params"][0].dim() == 0:
                optimizable_tensors[group["name"]] = group["params"][0]
                continue
    ...
    def prune_large_gaussians(self, max_radius_threshold):
        prune_radii = self.max_radii2D.squeeze() > max_radius_threshold

        pruned_positions = self._xyz[prune_radii]
        pruned_scaling = self._scaling[prune_radii]
        pruned_features_dc = self._features_dc[prune_radii]
        pruned_features_rest = self._features_rest[prune_radii]
        pruned_opacities = self._opacity[prune_radii]
        pruned_rotation = self._rotation[prune_radii]

        self.prune_points(prune_radii)        
        return (pruned_positions, pruned_scaling, pruned_features_dc, 
                pruned_features_rest, pruned_opacities, pruned_rotation)
    
    def densify_around_pruned_points(self, pruned_positions, pruned_scaling, pruned_features_dc,
                                 pruned_features_rest, pruned_opacities, pruned_rotation, extent, N=1):
        if pruned_positions.size(0) == 0:
            return

        new_xyz = pruned_positions.repeat_interleave(N, dim=0)
        std_dev = 0.00001 * extent 
        perturbations = torch.randn_like(new_xyz) * std_dev
        new_xyz += perturbations

        new_scaling = pruned_scaling.repeat_interleave(N, dim=0) - 1.0  
        new_features_dc = pruned_features_dc.repeat_interleave(N, dim=0)
        new_features_rest = pruned_features_rest.repeat_interleave(N, dim=0)
        new_opacities = pruned_opacities.repeat_interleave(N, dim=0)
        new_rotation = pruned_rotation.repeat_interleave(N, dim=0)
